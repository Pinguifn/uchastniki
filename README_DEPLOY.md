...
Главная страница содержит адаптивную секцию `#interviews` между участниками и мероприятиями.

- Обложка встроена непосредственно в `index.html` в формате WebP, поэтому не зависит от отдельного файла.
- Короткое интро берётся по ссылке Google Drive и не хранится на сервере сайта.
- На компьютере интро запускается при наведении и останавливается при уходе курсора.
- На сенсорных устройствах первое нажатие запускает интро, повторное — останавливает.
- Полная версия открывается в Instagram по отдельной кнопке.

Для нового интервью добавьте синхронную пару элементов: `data-interview-copy` с текстом и `data-interview-media-slide` с обложкой и `video`. Карусель автоматически пересчитает количество слайдов, покажет стрелки и точки, а также включит перелистывание кнопками, клавиатурой и свайпом. Интро должно быть MP4/H.264, вертикальным 9:16, без обязательного звука и доступным по ссылке всем пользователям.
...
Оптимизированное видео хранится вне Git-репозитория на сервере:

```text
/var/www/uchastniki-media/interviews/katya-belka-intro.mp4
```

Nginx публикует его по адресу `/media/interviews/katya-belka-intro.mp4` с поддержкой Range-запросов. Файл не включается в архив сайта и загружается плеером только при взаимодействии.
