...
## Полные заявки в Telegram

По умолчанию Telegram получает только факт и дату заявки. Режим полного уведомления добавляет ФИО, телефон, email, Telegram-контакт, компанию и роль. Он включается только для новых заявок: сообщения, уже записанные в очередь, не изменяются. Версия согласия, принятого при отправке новой заявки, записывается вместе с ней в базе.

До включения режима опубликуйте обновлённые согласие и политики на сайте, проверьте фактическую инфраструктуру Telegram и выполните применимые требования к трансграничной передаче персональных данных, включая отдельное предварительное уведомление Роскомнадзора по ст. 12 Закона № 152-ФЗ. Сам текст согласия не заменяет эти действия. Ограничьте доступ к чату и переписке: сообщения остаются в Telegram по правилам сервиса, даже после удаления локальной записи очереди.

После этого задайте на сервере `TELEGRAM_INCLUDE_PERSONAL_DATA=true` и перезапустите приложение. Для Docker Compose добавьте эту строку в `.env`, затем выполните `docker compose up -d --build`; для запуска через systemd задайте переменную в конфигурации службы и перезапустите её. Проверьте в `/healthz` значение `"telegramPersonalDataEnabled":true`. Не включайте режим в публичном окружении до завершения указанных действий.
...
Параметр `"telegramPersonalDataEnabled"` показывает, включены ли полные уведомления. Его значение по умолчанию — `false`.
...
Документы синхронизированы с полями формы и обоими режимами Telegram-уведомлений. Перед публикацией подтвердите актуальность КПП 590401001, юридического адреса и регистрационного номера Роскомнадзора 78-25-083276. До включения полных уведомлений выполните действия, перечисленные в разделе «Полные заявки в Telegram».
