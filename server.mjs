...
// Удалённые SEO-страницы. Отдаём 410 Gone, чтобы поисковики убрали их из индекса.
const removedPaths = new Set([
  '/soobshchestvo-predprinimateley-perm',
  '/biznes-klub-perm',
  '/meropriyatiya-dlya-predprinimateley',
  '/rezidenty',
  '/stati',
  '/kak-vybrat-soobshchestvo-predprinimateley-perm',
  '/zachem-predprinimatelyu-delovoe-okruzhenie',
  '/gde-nayti-biznes-partnerov-v-permi',
  '/chto-takoe-biznes-klub',
  '/netvorking-dlya-predprinimateley',
  '/seo-pages.css'
]);
...
function validateMembership(input) {
  ...
  if (!values.consent) errors.consent = 'Нео��ходимо согласие на обработку данных.';
...
function sendGone(req, res) {
  const body = Buffer.from('<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="robots" content="noindex"><title>Страница удалена</title></head><body><h1>Страница удалена</h1><p>Этот раздел больше не существует. Перейдите на <a href="/">главную страницу</a>.</p></body></html>', 'utf8');
  res.writeHead(410, {...securityHeaders(), 'Content-Type':'text/html; charset=utf-8', 'Cache-Control':'no-store', 'X-Robots-Tag':'noindex', 'Content-Length':body.length});
  if (req.method === 'HEAD') res.end(); else res.end(body);
  return true;
}
